...
var LogEventRegistry = map[string]LogEventMeta{
	...
	"plugin.degraded":             {Name: "plugin.degraded", Summary: "A plugin is running but not fully working — either its RPC channel hit the consecutive-timeout threshold (reason absent), or the plugin reported the fault itself via plugin.report_health (reason present).", Since: "0.1.0", Source: "plugins", Severity: "warn", Redaction: "none"},
