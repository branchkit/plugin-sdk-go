...
// worker goroutine, so a plugin's listeners observe notifications in wire
// order. Unbounded + non-blocking push is
...
type Plugin struct {
	...
	// listeners observe them in wire order, one at a time (a slow listener
	// delays later events by design).
	notifyQ *notifyQueue
...
// next notification. Responses are matched inline in the read loop, so
// serializing here cannot deadlock.
...
func (p *Plugin) dispatch(msg rpcMessage) {
	...
	// order. push never blocks, so the
